...
public class FlashSaleProductServiceImpl implements FlashSaleProductService {
    ...
    public void createFlashSaleProduct(FlashSaleProductEvent event) {
        ...
        if (productVariant != null && productVariant.getProductVariantAttributeValues() != null) {
            for (ProductView.ProductVariantAttributeValue variantAttrValue : productVariant.getProductVariantAttributeValues()) {
                productView.getProductAttributes().stream()
                    ...
                    .ifPresent(attribute -> {
                        attribute.getProductAttributeValues().stream()
                            .filter(attrVal -> attrVal.get_id().equals(variantAttrValue.getProductAttributeValueId()))
                            .findFirst()
    ...
    public void updateFlashSaleProduct(FlashSaleProductEvent event) {
        ...
        flashSaleProductView.setOriginalPrice(event.getOriginalPrice());
    ...
    public PageResponse<FlashSaleProductView> getFlashSaleProducts(
        ...
        String ownerId = (isOwner != null && isOwner) ? shopId : null;
        ...
        return PageResponse.<FlashSaleProductView>builder()
            .data(page.getContent().stream().peek(flashSaleProductView ->
                flashSaleProductView.getProductImages().forEach(image ->
                    image.setImageUrl(fileService.getPresignedUrl(image.getImageUrl()))
                )
            ).toList())
