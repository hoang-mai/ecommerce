...
public class OrderEventProducer {
    ...
    public void send(CreatePaymentEvent createPaymentEvent) {
        createPaymentEventKafkaListener.send(CREATE_PAYMENT_NOTIFICATION_TOPIC, createPaymentEvent.getUserId(),
                createPaymentEvent);
    }

    public void sendUpdatePaymentStatusEvent(CreateListOrderStatusEvent createListOrderStatusEvent) {
        orderStatusEventKafkaTemplate.send(ORDER_STATUS_TOPIC, createListOrderStatusEvent.getUserId(),
                createListOrderStatusEvent);
        orderStatusEventKafkaTemplate.send(UPDATE_ORDER_STATUS_TOPIC, createListOrderStatusEvent.getUserId(),
                createListOrderStatusEvent);
    }
    ...
    private final KafkaTemplate<Long, RestoreFlashSaleStockEvent> restoreFlashSaleStockEventKafkaTemplate;

    public void sendRestoreFlashSaleStockEvent(RestoreFlashSaleStockEvent event) {
        restoreFlashSaleStockEventKafkaTemplate.send(RESTORE_FLASH_SALE_STOCK_TOPIC, event.getUserId(), event);
    }
