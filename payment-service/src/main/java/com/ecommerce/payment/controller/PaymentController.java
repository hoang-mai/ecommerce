...
public class PaymentController {
    ...
    public void ipn(
        ...
        paymentService.handleIPN(amount, bankCode, bankTranNo, cardType, orderInfo, payDate, responseCode, tmnCode,
                transactionNo, transactionStatus, txnRef, secureHash);
    }
    ...
    public ResponseEntity<BaseResponse<Void>> cancelledOrRefundPayment(@PathVariable Long orderId,
        ...
        paymentService.cancelledOrRefundPayment(orderId, updatePaymentStatusDTO.getReason(),
                updatePaymentStatusDTO.getPaymentStatus());
        return ResponseEntity.ok(
                BaseResponse.<Void>builder()
                        .message(messageService
                                .getMessage(PaymentStatus.CANCELLED == updatePaymentStatusDTO.getPaymentStatus()
                                        ? MessageSuccess.CANCELLED_SUCCESS
                                        : MessageSuccess.REFUND_SUCCESS))
                        .statusCode(200)
                        .build());
    }
}
